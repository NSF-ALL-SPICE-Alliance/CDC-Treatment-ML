...
```{r, include=FALSE}
freq_use_states$stfips_new <- sprintf('%02d', freq_use_states$stfips)
```

Change column name to match map

```{r, include=FALSE}
colnames(freq_use_states)[colnames(freq_use_states) == "stfips_new"] <- "state_fips"
```
...
ggplot(freq_use_states) +
  geom_sf(mapping = aes(fill = percentage),
          color = "#ffffff", size = 0.25) +
  labs(fill = "% of Treatment Episodes") +
   coord_sf(datum = NA)+
  #geom_sf_text(data = get_urbn_labels(map = "territories_states", sf = TRUE), 
...
```{r}
teds_d_select_years_relevant_completed$stfips <- sprintf('%02d',teds_d_select_years_relevant_completed$stfips)

colnames(teds_d_select_years_relevant_completed)[colnames(teds_d_select_years_relevant_completed) == "stfips"] <- "state_fips"
```
